...
import { deleteCondominium } from '@/lib/actions/condominium.actions';
...
export default function CondominiumTable({ condominiums }: Props) {
  ...
  return (
    <div className="bg-white border border-[#001629]/5 overflow-hidden">
      <div className="grid grid-cols-[auto_1fr_140px_120px_100px_80px_180px] gap-0 px-6 py-4 border-b border-[#001629]/5 bg-[#001629]/[0.02]">
        {['', 'Condomínio', 'Disposição', 'Localização', 'Publicado', 'Destaque', 'Ações'].map((h) => (
      ...
      {condominiums.map((c) => {
        const mainImage = c.images?.find((img) => img.isMain)?.url || c.images?.[0]?.url;
        ...
        return (
          <div
            key={c._id}
            className={`grid grid-cols-[auto_1fr_140px_120px_100px_80px_180px] gap-0 px-6 py-4 border-b border-[#001629]/5 items-center transition-colors ${
          ...
          >
            <div className="relative w-12 h-10 mr-4 bg-[#001629]/5 overflow-hidden shrink-0">
              {mainImage ? (
                <CloudinaryImage size="thumbnail" src={mainImage} alt={c.name} fill className="object-cover" />
              ) : (
            ...
            <div className="min-w-0 pr-4">
              <p className="font-noto text-sm text-[#001629] truncate leading-tight">{c.name}</p>
              <p className="font-manrope text-[10px] text-[#001629]/30 mt-0.5">{formatDate(c.createdAt)}</p>
            </div>

            <span className="font-manrope text-[10px] uppercase tracking-widest text-[#001629]/60">
              {c.disposition === 'horizontal' ? 'Horizontal' : 'Vertical'}
            </span>

            <span className="font-manrope text-[10px] text-[#001629]/60 truncate">{c.location}</span>

            <div className="flex justify-center">
              <span className={`w-2 h-2 rounded-full ${c.isPublished ? 'bg-emerald-400' : 'bg-[#001629]/20'}`} />
            </div>

            <div className="flex justify-center">
              <span className={`w-2 h-2 rounded-full ${c.isFeatured ? 'bg-[#775a19]' : 'bg-[#001629]/20'}`} />
            </div>
            ...
            <div className="flex items-center gap-2 justify-end">
              {isConfirming ? (
                ...
                <>
                  ...
                  <a
                    href={`/admin/condominiums/${c._id}/edit`}
