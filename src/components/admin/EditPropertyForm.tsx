...
export default function EditPropertyForm({ property }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    advancedLocation: {
      distanceToSea: String(property.advancedLocation?.distanceToSea || ''),
  ...
  return (
    <div className="min-h-screen bg-white py-20 px-8 lg:px-24">
      ...
      <div className="max-w-4xl mx-auto mb-16">
        ...
        <h1 className="font-noto text-5xl text-[#002B49] mb-3 tracking-tighter">Editar Patrimônio</h1>
        <p className="font-manrope text-sm text-[#002B49]/40 tracking-widest uppercase">{property.title}</p>
      </div>

      {/* Step current title summary moved to photo uploader section if needed, 
          but we remove the static preview now */}
          ...
          >
            ...
            <span className="relative z-10 flex items-center justify-center gap-3 font-noto text-[10px] text-white uppercase tracking-[0.4em]">
              ...
              {loading 
                ...
                : step === TOTAL_STEPS ? 'Salvar Alterações' : 'Prosseguir'}
            </span>
          </button>
        </div>
      </div>
