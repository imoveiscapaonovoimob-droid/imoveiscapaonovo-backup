...
export async function updateProperty(id: string, formData: any) {
  try {
    ...
    for (const img of images || []) {
      if (img.data) {
        const res = await cloudinary.uploader.upload(img.data, {
          folder: 'imoveis-capao-novo',
          resource_type: 'image',
        });
        newImages.push({ url: res.secure_url, public_id: res.public_id, isMain: img.isMain });
      } else if (img.url) {
        // Keep existing image
        newImages.push(img);
      }
    ...
    const updatedImages = newImages.length > 0 ? newImages : existing.images;
